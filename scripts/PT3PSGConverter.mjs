...
function parsePT3(data, sourceFile) {
  ...
  const noteTable = data[99] >= NOTE_TABLE.PROTRACKER && data[99] <= NOTE_TABLE.NATURAL ? data[99] : NOTE_TABLE.PROTRACKER;
  ...
  for (let index = deferred.length - 1; index >= 0; index -= 1) {
    ...
    switch (command) {
      case 1:
        ...
        break;
      case 2:
        cell.commands.push({ type: COMMAND.GLISS_NOTE, p1: data[state.offset++], p2: readInt16LE(data, state.offset + 2), p3: 0 });
        state.offset += 4;
        break;
      case 3:
...
function ascii(data, offset, length) {
  return Buffer.from(data.subarray(offset, offset + length)).toString('latin1').replace(/\0/g, '');
}
